...
def sample_particles_sim(env, cam_params, k_fps_particles, n_particles=2000):
...
class Planner(object):
    ...
    def trajectory_optimization(self, iter, ros_correction_path):
        ...
        if self.args.control_algo == 'fix':
            ...
            init_pose_seq, act_seq, loss_seq = self.trajectory_optimization_with_horizon(self.args.n_grips, state_cur)
    ...
    def visualize_results(self, initial_state, init_pose_seq, act_seq, state_goal, i):
        ...
        _, sim_state_seq = self.sim_rollout(init_pose_seq.unsqueeze(0), act_seq.unsqueeze(0))
        ...
        plt_render([sim_state_seq], state_goal[0], self.n_particle, os.path.join(self.rollout_path, f'anim_{i}.gif'))
    ...
    def rollout(self, init_pose_seqs_pool, act_seqs_pool, state_cur, state_goal):
        ...
        for i, _ in enumerate(batches):
            ...
            _, state_seqs = self.sim_rollout(init_pose_seqs, act_seqs)
            # else:
    ...
    def sim_rollout(self, init_pose_seqs, act_seqs):
        ...
        for t in range(act_seqs.shape[0]):
            ...
            # sample_state = sample_particles_sim(self.taichi_env, self.cam_params, self.n_particle)
            # state_seq_sample = []
...
def main():
    ...
    ros_correction_path = "/scr/hxu/catkin_ws/src/robocook_ros/dataset/"\
        + f"ngrip_fixed_robot_3-29/30-Mar-2022-23:21:01.687433/plasticine_{iter}.bag"
    ...
    if os.path.exists(ros_correction_path):
        ...
        with open(f"{control_out_dir}/init_pose_seq_{iter}.npy", 'wb') as f:
