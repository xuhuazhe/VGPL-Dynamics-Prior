...
task_params = {
    ...
    # "gripper_gap_limits_large": np.array([0.14, 0.06]),
    "p_noise_scale": 0.03,
    "p_noise_bound": 0.1,
...
def visualize_points_helper(ax, all_points, n_particles, p_color='b'):
    points = ax.scatter(all_points[:n_particles, 0], all_points[:n_particles, 2], all_points[:n_particles, 1],
                        c=p_color, s=10)
...
def get_center_and_rot_from_pose(init_pose_seq):
    ...
    mid_point_seq = (init_pose_seq[:, task_params["gripper_mid_pt"], :3] + init_pose_seq[:,
                                                                           task_params["gripper_mid_pt"], 7:10]) / 2
...
class Planner(object):
    def __init__(self, args, taichi_env, env_init_state, scene_params, n_particle, n_shape, model_small, model_large, all_p,
    ...
    def trajectory_optimization(self):
        ...
        if self.args.control_algo == 'fix':
            best_init_pose_seq, best_act_seq, best_model_loss = self.trajectory_optimization_with_horizon(
                self.args.n_grips, self.args.correction)
            best_sim_loss = self.visualize_results(best_init_pose_seq, best_act_seq, state_goal_final,
                                                   self.args.n_grips)
            ...
            for grip_num in range(self.args.n_grips, 0, -1):
                ...
                print(
                    f"=============== With {grip_num} grips -> model_loss: {loss_seq}; sim_loss: {loss_sim} ===============")
            ...
            os.system(
                f"cp {os.path.join(self.rollout_path, f'anim_{best_idx}.gif')} {os.path.join(self.rollout_path, f'best_anim.gif')}")
            os.system(
            ...
            for i in range(n_iters):
                ...
                loss_sim = self.visualize_results(init_pose_seq, act_seq, tool_seq, state_goal_final, i)
    ...
    def trajectory_optimization_with_horizon(self, grip_num, correction, checkpoint=None):
        ...
        for i in range(grip_num):
            ...
            init_pose_seqs_pool_large, act_seqs_pool_large = self.sample_action_params(n_grips_sample, size='large')
            init_pose_seqs_pool_small, act_seqs_pool_small = self.sample_action_params(n_grips_sample, size='small')
            ...
            reward_seqs_large, model_state_seqs_large = self.rollout(init_pose_seqs_pool_large, act_seqs_pool_large, state_cur, state_goal, size='large')
            reward_seqs_small, model_state_seqs_small = self.rollout(init_pose_seqs_pool_small, act_seqs_pool_small, state_cur, state_goal, size='small')
            print('sampling large: max: %.4f, mean: %.4f, std: %.4f' % (
                torch.max(reward_seqs_large), torch.mean(reward_seqs_large), torch.std(reward_seqs_large)))
            print('sampling small: max: %.4f, mean: %.4f, std: %.4f' % (
                torch.max(reward_seqs_small), torch.mean(reward_seqs_small), torch.std(reward_seqs_small)))
            ...
            if self.args.opt_algo == 'max':
                ...
                for j in range(self.args.CEM_opt_iter):
                    ...
                    if j == self.args.CEM_opt_iter - 1:
                        ...
                        init_pose_seqs_pool, act_seqs_pool = self.optimize_action_CEM(init_pose_seqs_pool,
                                                                                      act_seqs_pool, reward_seqs)
                        reward_seqs, model_state_seqs = self.rollout(init_pose_seqs_pool, act_seqs_pool, state_cur,
                                                                     state_goal)
    ...
    def visualize_results(self, init_pose_seq, act_seq, tool_seq, state_goal, i):
        ...
        visualize_points(sample_state_seq[-1], self.n_particle,
                         os.path.join(self.rollout_path, f'sim_particles_final_{i}'))
    ...
    def get_state_goal(self, i):
        goal_idx = min((i + 1) * (task_params["len_per_grip"] + task_params["len_per_grip_back"]) - 1,
                       len(self.all_p) - 1)
        state_goal = torch.FloatTensor(self.all_p[goal_idx]).unsqueeze(0)[:, :self.n_particle, :]

        if len(self.args.goal_shape_name) > 0 and self.args.goal_shape_name != 'none' and self.args.goal_shape_name[
                                                                                          :3] != 'vid':
            state_goal = self.goal_shapes[i]

        return state_goal

    def sample_action_params(self, n_grips, size='large'):
        # np.random.seed(0)
        init_pose_seqs = []
        act_seqs = []
        n_sampled = 0
        while n_sampled < self.sample_size:
            ...
            for i in range(n_grips):
                ...
                if size == 'large':
                    gripper_rate = np.random.uniform(*task_params["gripper_rate_limits_large"])
                elif size == 'small':
                    gripper_rate = np.random.uniform(*task_params["gripper_rate_limits_small"])
                else:
                    raise NotImplementedError
    ...
    def rollout(self, init_pose_seqs_pool, act_seqs_pool, state_cur, state_goal, size='large'):
        # import pdb; pdb.set_trace()
        reward_seqs_rollout = []
        state_seqs_rollout = []
        ...
        for i, _ in enumerate(batches):
            ...
            if self.args.use_sim:
                ...
                state_seqs = self.model_rollout(state_cur, init_pose_seqs, act_seqs, size=size)
                # state_seqs_small = self.model_rollout(state_cur, init_pose_seqs, act_seqs, size='small')

            reward_seqs = self.evaluate_traj(state_seqs, state_goal, self.args.reward_type)
            # reward_seqs_small = self.evaluate_traj(state_seqs_small, state_goal)
            # print(f"reward seqs: {reward_seqs}")
            # reward_seqs = reward_seqs.data.cpu().numpy()
            # if reward_seqs_large.item() >=  reward_seqs_small.item():
            #     reward_seqs_rollout.append(reward_seqs_large)
            #     state_seqs_rollout.append(state_seqs_large)
            #     chosen_one = 'large'
            # else:
            #     reward_seqs_rollout.append(reward_seqs_small)
            #     state_seqs_rollout.append(state_seqs_small)
            #     chosen_one = 'small'
        ...
        for t in range(act_seqs.shape[0]):
            ...
            for i in range(act_seqs.shape[1]):
                ...
                self.taichi_env.primitives.primitives[0].set_state(0,
                                                                   ...
                                                                   :7])
                self.taichi_env.primitives.primitives[1].set_state(0,
                                                                   init_pose_seqs[t, i, task_params["gripper_mid_pt"],
                                                                   7:])
    ...
    def model_rollout(
        ...
        memory_init = model.init_memory(init_pose_seqs.shape[0], self.n_particle + self.n_shape)
        ...
        for i in range(act_seqs.shape[1]):
            ...
            state_cur = torch.cat([state_cur[:, :, :self.n_particle, :], floor_state,
                                   shape1.unsqueeze(1).expand([-1, self.args.n_his, -1, -1]),
            ...
            for j in range(act_seqs.shape[2]):
                ...
                for k in range(len(Rr_curs)):
                    ...
                    Rr = torch.cat(
                        [Rr, torch.zeros((1, max_n_rel - Rr.size(1), self.n_particle + self.n_shape)).to(device)], 1)
                    Rs = torch.cat(
                        [Rs, torch.zeros((1, max_n_rel - Rs.size(1), self.n_particle + self.n_shape)).to(device)], 1)
                    Rn = torch.cat(
                        [Rn, torch.zeros((1, max_n_rel - Rn.size(1), self.n_particle + self.n_shape)).to(device)], 1)
                ...
                pred_pos = torch.cat(
                    [pred_pos, state_cur[:, -1, self.n_particle: self.n_particle + task_params["n_shapes_floor"], :],
                     shape1, shape2], 1)
    ...
    def optimize_action_CEM(  # Cross Entropy Method (CEM)
        ...
        visualize_sampled_init_pos(init_pose_seqs, reward_seqs, idx, \
                                   os.path.join(self.rollout_path,
                                                f'plot_cem_s{self.grip_cur}_o{self.CEM_opt_iter_cur}'))
        ...
        for i in range(best_k, 0, -1):
            ...
            if i > 1:
                ...
                n_init_pose_samples = self.CEM_init_pose_sample_size - len(init_pose_seqs_pool) // (
                self.CEM_gripper_rate_sample_size) + 1
    ...
    def optimize_action_GD(
        ...
        mid_point_x_bounds = [task_params["mid_point"][0] - task_params["p_noise_bound"],
                              task_params["mid_point"][0] + task_params["p_noise_bound"]]
        mid_point_z_bounds = [task_params["mid_point"][2] - task_params["p_noise_bound"],
                              task_params["mid_point"][2] + task_params["p_noise_bound"]]
        ...
        for b in range(n_batch):
            ...
            optimizer = torch.optim.LBFGS([mid_points, angles, gripper_rates], lr=lr, tolerance_change=1e-5,
            ...
            def closure():
                ...
                for i in range(mid_points[start_idx:end_idx].shape[0]):
                    ...
                    for j in range(mid_points.shape[1]):
                        # pdb.set_trace()
                        mid_point_clipped_x = torch.clamp(mid_points[start_idx + i, j, 0], min=mid_point_x_bounds[0],
                                                          max=mid_point_x_bounds[1])
                        mid_point_clipped_z = torch.clamp(mid_points[start_idx + i, j, 2], min=mid_point_z_bounds[0],
                                                          max=mid_point_z_bounds[1])
                ...
                # pdb.set_trace()
                reward_seqs, model_state_seqs = self.rollout(init_pose_seq_samples,
                                                             act_seq_samples[:, :, :task_params["len_per_grip"], :],
                                                             state_cur, state_goal)
            ...
            model_state_seq_list = torch.cat(
                (model_state_seq_list, model_state_seqs)) if model_state_seq_list != None else model_state_seqs
        ...
        for i in range(mid_points[idx[-1]].shape[0]):
            mid_point_clipped_x = torch.clamp(mid_points[idx[-1], i, 0], min=mid_point_x_bounds[0],
                                              max=mid_point_x_bounds[1])
            mid_point_clipped_z = torch.clamp(mid_points[idx[-1], i, 2], min=mid_point_z_bounds[0],
                                              max=mid_point_z_bounds[1])
...
def load_models(args, outf_new=None):
    # load dynamics model
    model = Model(args, use_gpu)
    model_name = 'net_epoch_%d_iter_%d.pth' % (args.eval_epoch, args.eval_iter)
    if outf_new:
        model_path = os.path.join(outf_new, model_name)
    else:
    ...
    # model_dict_large = model_large.state_dict()
...
def main():
    ...
    # need to load both tools ...
    # task_params['tool_size_small'] = 0.03
    # task_params['tppl_size_large'] = 0.045

    task_params["gripper_rate_limits_small"] = [
        (task_params['sample_radius'] * 2 - (task_params['gripper_gap_limits'][0] + 2 * task_params['tool_size_small'])) / (
        2 * task_params['len_per_grip']),
        (task_params['sample_radius'] * 2 - (task_params['gripper_gap_limits'][1] + 2 * task_params['tool_size_small'])) / (
        2 * task_params['len_per_grip'])
    ]
    task_params["gripper_rate_limits_large"] = [
        (task_params['sample_radius'] * 2 - (
        task_params['gripper_gap_limits'][0] + 2 * task_params['tool_size_large'])) / (
            2 * task_params['len_per_grip']),
        (task_params['sample_radius'] * 2 - (
        task_params['gripper_gap_limits'][1] + 2 * task_params['tool_size_large'])) / (
            2 * task_params['len_per_grip'])
    ]
    ...
    if platform != 'darwin':
        ...
        def update_camera(env):
            env.renderer.camera_pos[
                0] = 0.5  # np.array([float(i) for i in (0.5, 2.5, 0.5)]) #(0.5, 2.5, 0.5)  #.from_numpy(np.array([[0.5, 2.5, 0.5]]))
            env.renderer.camera_pos[1] = 2.5
    ...
    model_small = load_models(args, args.outf_new)
    ...
    for t in range(args.time_step):
        ...
        if t % steps_per_grip == 0:
            init_pose_gt.append(
                np.concatenate((states[g1_idx: g2_idx], unit_quat_pad, states[g2_idx:], unit_quat_pad), axis=1))
        ...
        if len(actions) == steps_per_grip:
            ...
            # import pdb; pdb.set_trace()
            # hard code
            init_pose_gt[-1] = np.concatenate((init_pose_gt[-1][:, :3] - 2 * 0.02 * np.tile(actions[0][:3], (
            init_pose_gt[-1].shape[0], 1)), unit_quat_pad, \
                                               init_pose_gt[-1][:, 7:10] - 2 * 0.02 * np.tile(actions[0][6:9], (
                                               init_pose_gt[-1].shape[0], 1)), unit_quat_pad), axis=1)
            actions = []
    ...
    planner = Planner(args=args, taichi_env=env, env_init_state=state, scene_params=scene_params, n_particle=n_particle,
                      n_shape=n_shape, model_small=model_small, model_large=model_large, all_p=all_p, cam_params=cam_params, goal_shapes=goal_shapes,
                      task_params=task_params, use_gpu=use_gpu,
