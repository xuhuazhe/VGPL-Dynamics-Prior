...
class Evaluator():
    def __init__(self, args, eval_epoch, eval_iter):
        self.args = args

        self.evalf = os.path.join(self.args.outf, 'eval')

        os.system('mkdir -p ' + self.evalf)
        os.system('mkdir -p ' + os.path.join(self.evalf, 'plot'))
        os.system('mkdir -p ' + os.path.join(self.evalf, 'render'))
        os.system('mkdir -p ' + os.path.join(self.evalf, 'vispy'))

        tee = Tee(os.path.join(self.evalf, 'eval.log'), 'w')

        self.data_names = self.args.data_names
        self.use_gpu = 0 #torch.cuda.is_available()

        # create model and load weights
        self.model = Model(self.args, self.use_gpu)
        ...
        model_path = os.path.join(self.args.outf, model_name)    # self.args.outf
        ...
        if self.args.stage == 'dy':
            ...
            AssertionError("Unsupported stage %s, using other evaluation scripts" % self.args.stage)
        ...
        self.uh_loss = UpdatedHausdorffLoss()
    ...
    def evaluate(self):
        ...
        for idx_episode in range(0, self.n_episodes, 1): 
            emd_list = []
            print(f"Rollout {idx_episode} / {self.n_episodes}")
            ...
            p_gt = []
            s_gt = []
            for step in range(self.args.time_step):
                data_path = os.path.join(self.args.dataf, 'train', str(idx_episode).zfill(3), str(step) + '.h5')
            ...
            p_pred = torch.zeros(self.args.time_step, n_particle + n_shape, self.args.state_dim)
            # initialize particle grouping
            group_gt = get_env_group(self.args, n_particle, scene_params, use_gpu=self.use_gpu)
            ...
            memory_init = self.model.init_memory(B, n_particle + n_shape)

            # model rollout
            loss = 0.
            loss_raw = 0.
            loss_counter = 0.
            st_idx = self.args.n_his
            ed_idx = self.args.time_step
            ...
            with torch.set_grad_enabled(False):
                ...
                for step_id in range(st_idx, ed_idx):
                    ...
                    if step_id == st_idx:
                        ...
                        state_cur = p_gt[step_id - self.args.n_his:step_id]
                        if self.use_gpu:
                    ...
                    # state_cur (unnormalized): n_his x (n_p + n_s) x state_dim
                    attr, _, Rr_cur, Rs_cur, cluster_onehot = prepare_input(state_cur[-1].cpu().numpy(), n_particle, n_shape, self.args, stdreg=self.args.stdreg)
                    if self.use_gpu:
                        attr = attr.cuda()
                        Rr_cur = Rr_cur.cuda()
                        Rs_cur = Rs_cur.cuda()
                    ...
                    mean_d, std_d = self.model.stat[2:]
            ...
            plot_curves(emd_list, path=os.path.join(self.evalf, 'plot', 'loss_curves_%d.png' % (idx_episode)))
            # import pdb; pdb.set_trace()
            '''
            visualization
            '''
            ...
            if self.vispy:
                ...
                if self.args.env == 'Pinch':
                    ...
                    # set instance colors
                    instance_colors = create_instance_colors(self.args.n_instance)

                    # render floor
                    add_floor(view)

                if self.args.env == 'Gripper':
                ...
                def update(event):
                    ...
                    if t_step < vis_length:
                        ...
                        colors = convert_groups_to_colors(
                            ...
                            instance_colors=instance_colors, env=self.args.env)
                ...
                if self.args.stage in ['dy']:
                    ...
                    out = cv2.VideoWriter(
                        os.path.join(self.evalf, 'render', 'vid_%d_vispy.avi' % (idx_episode)),
                        fourcc, 20, (800 * 2, 600))

                    for step in range(vis_length):
                        gt_path = os.path.join(self.evalf, 'vispy', 'gt_%d_%d.png' % (idx_episode, step))
                        pred_path = os.path.join(self.evalf, 'vispy', 'pred_%d_%d.png' % (idx_episode, step))
        ...
        with open(os.path.join(self.evalf, "stats.txt"), 'w') as file:
